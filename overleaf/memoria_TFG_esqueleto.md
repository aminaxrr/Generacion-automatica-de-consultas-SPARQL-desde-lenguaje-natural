<!--
Notas internas (no imprimir en la versión final):

- Memoria técnica para un sistema offline y determinista de traducción texto → SPARQL sobre un grafo RDF.
- Estilo de citas: IEEE numérico [1], [2], ...
- Objetivo orientativo de extensión: 70–90 páginas.

Los datos de portada (titulación, curso, autor/a, tutor/a, lugar, fecha, licencia) se gestionan en overleaf/main.tex.
-->
...
1) **Ausencia de evidencia (incompletitud).**
	...
	- Uso típico: “requisitos sin modelo”, “links sin description”, “escenarios sin Verified_by ni Validated_by”.
...
2) **Agregación y distribución (resúmenes).**
	...
	- Uso típico: “cuántos X”, “distribución por maturity”, “modelos por proveedor”.
...
3) **Redundancia e integridad (duplicados).**
	...
	- Uso típico: “links duplicados”, “trazas repetidas”.
...
4) **Coherencia de metadatos (reglas de calidad).**
	...
	- Uso típico: “contentType incoherente”, “timestamps obligatorios ausentes”.
...
- **Oráculo por propiedades (metas de auditoría).** En auditorías, a veces basta con validar propiedades: por ejemplo, que el resultado incluya únicamente link nodes sin timestamps, o que un duplicado tenga `COUNT>1`. Esta opción no reemplaza al oráculo denotacional, pero ayuda a explicar “por qué es correcto”.
...
Para que la herramienta sea útil en auditoría, la salida no debe limitarse a un texto “interpretativo”. El sistema debe recibir como entrada una pregunta en lenguaje natural (en español o inglés, según configuración) y producir como salida: (i) una consulta SPARQL de solo lectura (`SELECT`) alineada con el esquema del grafo, (ii) el resultado obtenido al ejecutarla localmente, y (iii) una traza de explicación que permita entender el operador elegido y el patrón de consulta aplicado.
...
Un elemento relevante del modelado es el uso de **nodos intermedios de enlace** (*link nodes*). En lugar de expresar una relación como un triple directo `A → B`, se introduce un nodo `L` que permite añadir metadatos a la relación (por ejemplo, tipo de enlace, contentType, timestamp o description). Este patrón habilita auditorías como “enlaces sin timestamp” o “enlaces con contentType incoherente”.
...
- **Ausencia de metadatos en la traza** (auditoría de calidad): “links sin timestamps”. Se formula como ausencia de un patrón sobre el nodo `?ln`:
	- patrón típico: `FILTER NOT EXISTS { ?ln p510:Timestamp_PLM ?ts }` (o la combinación que aplique) [3].
- **Incoherencia de metadatos**: “contentType incoherente”. Se formula como un filtro sobre `?ln p510:ContentType ?ct` (y, si se requiere, condiciones adicionales sobre origen/destino), evitando inferencias no verificables.
...
- Ejemplos de auditoría:
	- links sin timestamp: `q13_links_sin_timestamp.sparql`
	- links sin description: `q25_links_sin_description.sparql`
...
|---|---|---|---|
| `q1_req_sin_modelo_fisico.sparql` | Cobertura de trazabilidad | `FILTER NOT EXISTS` | `Requirement` sin `Satisfied_by`→`ln` (CT="Physical Model")→`Link`→`DesignModel` |
| `q2_modelos_sin_test.sparql` | Cobertura de verificación | `FILTER NOT EXISTS` | `DesignModel` (CT="Physical Model") sin `Verified_by`→`ln` (CT="Test Case")→`Link`→`Test` |
...
| `q12_vnv_escenarios_incompletos.sparql` | Integridad V&V | doble `FILTER NOT EXISTS` | `Scenario` sin `Verified_by` y sin `Validated_by` |
| `q13_links_sin_timestamp.sparql` | Calidad de link node | `FILTER` con `!EXISTS` + OR | `Traceability_Link_Type` sin `Timestamp_Archiving` o sin `Timestamp_PLM` |
...
| `q15_modelos_sin_proveedor.sparql` | Gobierno / completitud | `FILTER NOT EXISTS` | `DesignModel` (CT="Physical Model") sin `ex:providedBy` |
| `q16_requisitos_sin_aprobador.sparql` | Gobierno / completitud | `OPTIONAL` + `FILTER NOT EXISTS` | `Requirement` (estado opcional) sin `Approver` |
...
| `q19_proveedor_top_modelos_sin_test.sparql` | Riesgo por proveedor | `FILTER NOT EXISTS` + `GROUP BY` | Por `prov`: contar `DesignModel` sin `Verified_by` a `Test Case` |
...
| `q22_aprobados_sin_aprobador.sparql` | Gobierno (regla compuesta) | `UNION` + `FILTER NOT EXISTS` | `Requirement` o `DesignModel` con `Approval_State="Approved"` sin `Approver` |
...
| `q25_links_sin_description.sparql` | Calidad de link node | `FILTER NOT EXISTS` | `Traceability_Link_Type` sin `Description` |
...
**`q2_modelos_sin_test.sparql` — Modelos físicos sin test asociado.**
- Intención: “models without tests / sin verificación”.
- Operador SPARQL: ausencia con `FILTER NOT EXISTS` [3].
- Núcleo de patrón:
...
**`q13_links_sin_timestamp.sparql` — Links sin timestamps.**
...
**`q25_links_sin_description.sparql` — Links sin description.**
...
**`q16_requisitos_sin_aprobador.sparql` — Requisitos sin aprobador.**
...
**`q22_aprobados_sin_aprobador.sparql` — Approved sin Approver (Requirements y Models).**
...
**`q15_modelos_sin_proveedor.sparql` — Modelos físicos sin proveedor.**
...
**`q19_proveedor_top_modelos_sin_test.sparql` — Proveedores con más modelos sin test.**
...
#### 4.2.7 Checker anti-invención como “gate” obligatorio antes de ejecutar
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.85\linewidth,height=0.7\textheight,keepaspectratio]{figures/ui_streamlit_end2end.png}
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.85\linewidth,height=0.7\textheight,keepaspectratio]{figures/ui_streamlit_duplicates.png}
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.85\linewidth,height=0.7\textheight,keepaspectratio]{figures/ui_streamlit_explain_end2end.png}
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.85\linewidth,height=0.7\textheight,keepaspectratio]{figures/ui_streamlit_explain_duplicates.png}
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.85\linewidth,height=0.7\textheight,keepaspectratio]{figures/cli_explain_end2end.png}
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.85\linewidth,height=0.7\textheight,keepaspectratio]{figures/cli_explain_duplicates.png}
...
|---|---|---|
| `q1_req_sin_modelo_fisico.sparql` | Requisitos sin modelo físico | `FILTER NOT EXISTS` |
| `q2_modelos_sin_test.sparql` | Modelos sin test asociado | `FILTER NOT EXISTS` |
| `q3_porcentaje_req_con_modelo.sparql` | Porcentaje de requisitos con modelo | `COUNT` / agregación |
| `q4_req_sin_traza_end_to_end.sparql` | Requisitos sin traza end-to-end (Req→Model→Test) | `FILTER NOT EXISTS` |
| `q5_reqs_sobre_especificados.sparql` | Requisitos con más de un modelo físico | `GROUP BY` + `HAVING` |
| `q6_cuantos_proveedores.sparql` | Cuántos proveedores | `COUNT` |
| `q7_modelos_por_proveedor.sparql` | Modelos por proveedor | `GROUP BY` + `COUNT` |
| `q8_plm_resumen.sparql` | Resumen PLM del manifest | `OPTIONAL` / proyección |
| `q9_dev_environment.sparql` | Entorno de desarrollo del manifest | `OPTIONAL` |
| `q10_documentos_usados.sparql` | Documentos usados en el entorno | `OPTIONAL` |
| `q11_vnv_escenarios_resumen.sparql` | Resumen de escenarios V&V | `OPTIONAL` |
| `q12_vnv_escenarios_incompletos.sparql` | Escenarios sin Verified_by ni Validated_by | `FILTER NOT EXISTS` |
| `q13_links_sin_timestamp.sparql` | Links sin timestamps obligatorios | `EXISTS`/`FILTER` (ausencia) |
| `q14_conteo_entidades.sparql` | Conteo global por tipo de entidad | `GROUP BY` + `COUNT` |
| `q15_modelos_sin_proveedor.sparql` | Modelos sin proveedor | `FILTER NOT EXISTS` |
| `q16_requisitos_sin_aprobador.sparql` | Requisitos sin aprobador | `FILTER NOT EXISTS` |
| `q17_requisitos_por_maturity.sparql` | Requisitos por estado de madurez | `GROUP BY` + `COUNT` |
| `q18_requisitos_por_org_autora.sparql` | Requisitos por organización autora | `GROUP BY` + `COUNT` |
| `q19_proveedor_top_modelos_sin_test.sparql` | Proveedores con más modelos sin verificación | `FILTER NOT EXISTS` + agregación |
| `q20_modelos_por_estado_aprobacion.sparql` | Modelos por estado de aprobación | `GROUP BY` + `COUNT` |
| `q21_tests_por_proveedor_via_modelo.sparql` | Tests por proveedor (vía modelos) | `GROUP BY` + `COUNT` |
| `q22_aprobados_sin_aprobador.sparql` | Elementos Approved sin Approver | `FILTER NOT EXISTS` |
| `q23_link_contenttype_incoherente.sparql` | Incoherencias de ContentType en links | `OPTIONAL` / filtros |
| `q24_links_duplicados.sparql` | Duplicados (misma src+pred+target repetida) | `GROUP BY` + `HAVING` |
| `q25_links_sin_description.sparql` | Links sin Description | `FILTER NOT EXISTS` |
| `q26_baseline_y_proyecto.sparql` | Baseline y proyecto del manifest | `OPTIONAL` |
| `q27_requisitos_por_subsistema.sparql` | Requisitos por subsistema | `GROUP BY` + `COUNT` |
| `q28_requisitos_por_metodo_verificacion.sparql` | Requisitos por método de verificación | `GROUP BY` + `COUNT` |
...
|---|---|---|---|---:|---|
| Auditoría: enlaces sin timestamps | `D.audit.links_missing_timestamps` | Ausencia de metadato (p. ej. `FILTER NOT EXISTS`) | `LINKS_MISSING_TIMESTAMP` | 3 | [eval/paraphrase_smoke_out_current_utf8.txt](eval/paraphrase_smoke_out_current_utf8.txt) |
...
| Modelos sin test (NOT EXISTS) | Modelos sin test asociado | “sin test”, “no verificado”, “models without tests” | `FILTER NOT EXISTS { ?m p510:Verified_by ?ln . ?ln p510:Link ?test . ... }` | `q2_modelos_sin_test.sparql` |
| Requisitos sin traza end-to-end (NOT EXISTS) | Requisitos sin cadena Req→Model→Test | “end-to-end”, “missing traceability”, “sin traza completa” | `FILTER NOT EXISTS { ?req p510:Satisfied_by ?lnm . ?lnm p510:Link ?m . ?m p510:Verified_by ?lnt . ?lnt p510:Link ?t . }` | `q4_req_sin_traza_end_to_end.sparql` |
...
| Conteo total (COUNT) | Contar entidades (suppliers, links, requisitos, etc.) | “cuántos”, “número de”, “how many” | `SELECT (COUNT(DISTINCT ?x) AS ?n) WHERE { ?x a ... }` | `q6_cuantos_proveedores.sparql` |
| Porcentaje (COUNT + expresión) | Porcentaje de requisitos con modelo | “percentage/ratio”, “porcentaje” | `SELECT (100*?n_with/?n_total AS ?pct) WHERE { ... }` | `q3_porcentaje_req_con_modelo.sparql` |
| Distribución por proveedor (GROUP BY) | Modelos por proveedor | “by supplier”, “por proveedor” | `GROUP BY ?supplier (COUNT(DISTINCT ?m) AS ?n)` | `q7_modelos_por_proveedor.sparql` |
| Distribución por aprobación (GROUP BY) | Modelos por estado de aprobación | “approval state”, “estado de aprobación” | `GROUP BY ?state (COUNT(DISTINCT ?m) AS ?n)` | `q20_modelos_por_estado_aprobacion.sparql` |
| Distribución por madurez (GROUP BY) | Requisitos por estado de madurez | “maturity state”, “por madurez” | `GROUP BY ?maturity (COUNT(DISTINCT ?req) AS ?n)` | `q17_requisitos_por_maturity.sparql` |
| Distribución por subsistema (GROUP BY) | Requisitos por subsistema | “subsystem”, “por subsistema” | `GROUP BY ?subsystem (COUNT(DISTINCT ?req) AS ?n)` | `q27_requisitos_por_subsistema.sparql` |
| Distribución por método V&V (GROUP BY) | Requisitos por método de verificación | “verification method”, “por método” | `GROUP BY ?method (COUNT(DISTINCT ?req) AS ?n)` | `q28_requisitos_por_metodo_verificacion.sparql` |
| Auditoría: links sin timestamp (NOT EXISTS) | Detectar trazas sin timestamps obligatorios | “missing timestamp”, “sin timestamps” | `FILTER NOT EXISTS { ?ln p510:Timestamp_PLM ?ts }` | `q13_links_sin_timestamp.sparql` |
| Auditoría: links sin description (NOT EXISTS) | Detectar trazas sin descripción | “without description”, “sin description” | `FILTER NOT EXISTS { ?ln p510:Description ?d }` | `q25_links_sin_description.sparql` |
| Auditoría: contentType incoherente (FILTER) | Detectar incoherencias en tipo de contenido | “contentType mismatch”, “incoherente” | `?ln p510:ContentType ?ct . FILTER(?ct != ...)` | `q23_link_contenttype_incoherente.sparql` |
| Auditoría: sin aprobador (NOT EXISTS) | Requisitos sin aprobador | “without approver”, “sin aprobador” | `FILTER NOT EXISTS { ?req p510:Approver ?a }` | `q16_requisitos_sin_aprobador.sparql` |
| Auditoría: Approved sin Approver (NOT EXISTS) | Estado Approved sin atributo Approver | “approved without approver” | `?x p510:Approval_State "Approved" . FILTER NOT EXISTS { ?x p510:Approver ?a }` | `q22_aprobados_sin_aprobador.sparql` |
